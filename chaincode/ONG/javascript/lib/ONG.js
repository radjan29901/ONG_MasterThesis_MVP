...
https://www.youtube.com/watch?v=LpC4JNKacww
https://stackoverflow.com/questions/50144119/fabric-docker-image-version-of-1-1-0-alpha-does-not-match-this-newer-version-of
 ...
 //Transform
     ...
     async queryAllDonationsAdmin(ctx) {
        const startKey = 'DONATION0';
        const endKey = 'DONATION999';
     ...
     async changeStatusDonation(ctx, idDonation, newStatus) {
        ...
        const donationAsBytes = await ctx.stub.getState(idDonation); // get the car from chaincode state
        if (!donationAsBytes || donationAsBytes.length === 0) {
            throw new Error(`${idDonation} does not exist`);
        }
       
        const donation = JSON.parse(donationAsBytes.toString());
        donation.type = newStatus;
    
        await ctx.stub.putState(idDonation, Buffer.from(JSON.stringify(donation)));
